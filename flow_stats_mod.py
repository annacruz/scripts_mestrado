...
def _timer_func():
    ...
    log.debug(
        "Sent %i flow/port stats request(s)",
...
def _handle_flowstats_received(event):
    ...
    log.info(
        "Traffic from %s: %s packets || \
        %s flows || %s dst port and %s src port \
        || %s dst ip and %s src ip || %s in_port",
        dpidToStr(event.connection.dpid), web_packet,
        web_flows, tp_dst, tp_src, nw_dst, nw_src, in_port)
